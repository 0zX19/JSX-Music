    ...
    run: async (client, message, args, user, language, prefix) => {
        ...
        try {
            ...
            if (user && user.isPremium) {
                ...
                return msg.edit({ content: " ", embeds: [premiumEmbed] });
            }
        } catch (err) {
            console.error(err);
            return msg.edit({ content: `${client.i18n.get(language, "music", "autoplay_error")}`, embeds: [new EmbedBuilder().setColor(client.color).setDescription(`Error: ${err.message}`)] });
        }
    }
};
