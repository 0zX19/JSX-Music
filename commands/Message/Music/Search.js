...
module.exports = { 
    ...
    run: async (client, message, args, user, language, prefix) => {
            const msg = await message.channel.send({ embeds: [new EmbedBuilder().setColor(client.color).setDescription(`${client.i18n.get(language, "music", "search_loading")}`)] })
        ...
        if (search.startsWith("https://") || search.startsWith("http://")) {
            if (!search.match(/(?:https:\/\/open\.spotify\.com\/|spotify:)(?:.+)?(track|playlist|album|artist)[\/:]([A-Za-z0-9]+)/)) {
                if (!search.match(/^https?:\/\/(soundcloud\.com|snd\.sc)\/(.*)$/)) {
                    return msg.edit({
                        embeds: [
                            new EmbedBuilder()
                            .setColor(client.color)
                            .setAuthor({ name: client.user.username, iconURL:client.user.displayAvatarURL({ forceStatic: true }),})
                            .setDescription(`We No Longer Support Youtube Links Due To Recent Activities. Use **Spotify**, **Soundcloud**, **Facebook**, **Deezer**, And **AppleMusic** . Otherwise Use A Search Query To Use Our Default System.`)
                        ]
                    })
                }
            }
